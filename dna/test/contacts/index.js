...
module.exports = (scenario, conductorConfig) => {
    ...
    scenario("block contact", async (s, t) => {
      ...
      //BLOCK OWN SELF
      const invalid_block_contact_result_0 = await blockContact("alice", 0)(alice);
      await s.consistency();
      t.deepEqual(invalid_block_contact_result_0.Err, {"Internal":"Cannot block yourself"});
      ...
      await s.consistency();
      t.deepEqual(invalid_block_contact_result_1.Err, {"Internal":"The timestamp is the same with or less than the previous timestamp"});

      //BLOCK AN ALREADY BLOCKED CONTACT
      const invalid_block_contact_result_2 = await blockContact("bob", 3)(alice);
      await s.consistency();
      t.deepEqual(invalid_block_contact_result_2.Err, {"Internal":"The contact is already in the list of blocked contacts"});
    })
    ...
    scenario("unblock contact", async (s, t) => {
      ...
      //UPDATE BLOCKED LIST WITH AN INVALID TIMESTAMP
      const invalid_unblock_contact_result_1 = await unblockContact("bob", 1)(alice);
      await s.consistency();
      t.deepEqual(invalid_unblock_contact_result_1.Err, {"Internal":"The timestamp is the same with or less than the previous timestamp"});

      //UNBLOCK AN UNBLOCKED CONTACT
      const invalid_unblock_contact_result_2 = await unblockContact("bob", 4)(alice);
      await s.consistency();
      t.deepEqual(invalid_unblock_contact_result_2.Err, {"Internal":"The contact is not in the list of blocked contacts"});
    })
}